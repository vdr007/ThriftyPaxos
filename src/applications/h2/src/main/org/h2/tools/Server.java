...
public class Server extends Tool implements Runnable, ShutdownHandler {
    ...
    public static Server createTcpServer(String... args) throws SQLException {
    	...
    	String hostname = "haha";
    	int rank = 1;
    	String config = "/users/shir/myproject/glassPaxos/glasspaxos.conf";
    	try{
    	//	glassPaxos.Configuration.initConfiguration(config);

	    String nodeName = InetAddress.getLocalHost().getHostName();
	    rank = Integer.parseInt(nodeName.substring(nodeName.length()-1));
	    System.out.println("nodeName = "+nodeName+" rank="+rank);
    	    Learner learner = new Learner();
            service = new TcpServer();
            service.setLibServerInterface(learner); //vanderong
            learner.setApplicationServer(service);
