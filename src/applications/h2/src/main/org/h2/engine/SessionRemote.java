...
import glassPaxos.apps.hashtable.AppClient;
 ...
 */
public class SessionRemote extends SessionWithState implements DataHandler, AppClientCallback {
    ...
    private Trace trace;
    private ArrayList<Client> transferList = New.arrayList();
    private int nextId;
    ...
    private SimpleLogger LOG = SimpleLogger.getLogger("h2.SessionRemote");
    ...
    public ArrayList<String> getClusterServers() {
        ...
        for (int i = 0; i < transferList.size(); i++) {
            ...
            serverList.add(transfer.toString());
    ...
    //private static Client tmp = null;
    private static int rank = 1;
    private final Object rankLock = new Object();
    private Client initTransfer(ConnectionInfo ci, String db, String server)
            throws IOException {
        ...
        trans.init();*/
    	//int rank = 1;
        String configFile ="/users/shir/myproject/glassPaxos/glasspaxos.conf";
        int vLen = 8;

        //AppClient app = new AppClient(200);

        NodeIdentifier clientID =
                new NodeIdentifier(NodeIdentifier.Role.CLIENT, rank);
        System.out.format("clientID %s\n", clientID.toString());
        Client trans = null;
	synchronized(rankLock){
		trans = new Client();
		try {
			trans.initEnv(rank, configFile);
		}
		catch(Exception e1) {
			e1.printStackTrace();
			System.exit(-1);
	...
	System.out.println("iodine Paxos client created");
    	byte[] req = new byte[65536];
    	ByteBuffer bb = ByteBuffer.wrap(req);
        bb.putInt(Constants.TCP_PROTOCOL_VERSION_6);
        bb.putInt(Constants.TCP_PROTOCOL_VERSION_15);
        ByteBufferTool.putString(bb, db);
        ByteBufferTool.putString(bb, ci.getOriginalURL());
        ByteBufferTool.putString(bb, ci.getUserName());
        ByteBufferTool.putBytes(bb, ci.getUserPasswordHash());
        ByteBufferTool.putBytes(bb, ci.getFilePasswordHash());
        String[] keys = ci.getKeys();
        bb.putInt(keys.length);
        for (String key : keys) {
        	ByteBufferTool.putString(bb, key);
        	ByteBufferTool.putString(bb, ci.getProperty(key));
        }
        this.requestStart();
        trans.sendRequest(ByteBufferTool.convertBB(bb), this);
	System.out.println("iodine initialization request sent");
        try {
            done(trans);
	    System.out.println("iodine initialization response received");
            clientVersion = retBB.getInt();
            trans.setProperty("clientVersion", clientVersion);
            if (clientVersion >= Constants.TCP_PROTOCOL_VERSION_14) {
                if (ci.getFileEncryptionKey() != null) {
                	bb = ByteBuffer.wrap(req);
                	ByteBufferTool.putBytes(bb, ci.getFileEncryptionKey());
                	this.requestStart();
                	trans.sendRequest(ByteBufferTool.convertBB(bb), this);
                    done(trans);
	    ...
	    //System.out.println("SESSION_SET_ID");
            bb = ByteBuffer.wrap(req);
            bb.putInt(SessionRemote.SESSION_SET_ID);
            ByteBufferTool.putString(bb, sessionId);
            ...
            if (clientVersion >= Constants.TCP_PROTOCOL_VERSION_15) {
                autoCommit = ByteBufferTool.getBoolean(retBB);
            } else {
                autoCommit = true;
            }
            return trans;
    ...
    public boolean hasPendingTransaction() {
        ...
        for (int i = 0, count = 0; i < transferList.size(); i++) {
            Client transfer = transferList.get(i);
            try {
		//System.out.println("iodine SESSION_HAS_PENDING_TRANSACTION");
                traceOperation("SESSION_HAS_PENDING_TRANSACTION", 0);
                byte[] req = new byte[4];
                ByteBuffer b = ByteBuffer.wrap(req);
                b.putInt(SessionRemote.SESSION_HAS_PENDING_TRANSACTION);
                this.requestStart();
                transfer.sendRequest(req, this);
                //transfer.writeInt(
                //        SessionRemote.SESSION_HAS_PENDING_TRANSACTION);
                done(transfer);
                return retBB.getInt() != 0;
            } catch (IOException e) {
    ...
    private void setAutoCommitSend(boolean autoCommit) {
        if (clientVersion >= Constants.TCP_PROTOCOL_VERSION_8) {
            for (int i = 0, count = 0; i < transferList.size(); i++) {
                Client transfer = transferList.get(i);
                try {
		    //System.out.println("iodine SESSION_SET_AUTOCOMMIT");
                    traceOperation("SESSION_SET_AUTOCOMMIT", autoCommit ? 1 : 0);
                    byte[] req = new byte[4 + 1];
                    ByteBuffer bb = ByteBuffer.wrap(req);
                    bb.putInt(SessionRemote.SESSION_SET_AUTOCOMMIT);
                    ByteBufferTool.putBoolean(bb, autoCommit);
    ...
    public void autoCommitIfCluster() {
        if (autoCommit && cluster) {
		    ...
		    System.out.println("iodine COMMAND_COMMIT");
                    ...
                    byte[] req = new byte[4];
                    ByteBuffer bb = ByteBuffer.wrap(req);
                    bb.putInt(SessionRemote.COMMAND_COMMIT);
    ...
    private void connectServer(ConnectionInfo ci) {
        ...
        try {
            for (int i = 0; i < len; i++) {
                String s = servers[i];
                try {
                	System.out.println("iodine initTransfer "+s);
                    Client trans = initTransfer(ci, databaseName, s);
        ...
        if (transferList != null) {
            synchronized (this) {
                for (Client transfer : transferList) {
                    try {
			//System.out.println("iodine SESSION_CLOSE");
                        traceOperation("SESSION_CLOSE", 0);
                        byte[] req = new byte[4];
                        ByteBuffer bb = ByteBuffer.wrap(req);
                        bb.putInt(SessionRemote.SESSION_CLOSE);
     ...
     *             and server
     */
    public void done(Client transfer) throws IOException {
    	synchronized(retLock){
    		...
    		retBB = ByteBuffer.wrap(retBuffer);
    		int status = retBB.getInt();
            if (status == STATUS_ERROR) {
                String sqlstate = ByteBufferTool.getString(retBB);
                String message = ByteBufferTool.getString(retBB);
                String sql = ByteBufferTool.getString(retBB);
                int errorCode = retBB.getInt();
                String stackTrace = ByteBufferTool.getString(retBB);
    ...
    private final Object retLock = new Object();
    private byte[] retBuffer = null;
    public ByteBuffer retBB = null;
	@Override
	public void requestComplete(byte[] req, boolean completed, byte[] ret) {
		//System.out.println("iodine receive requestComplete ret="+ret+" time="+System.currentTimeMillis());
		if(!completed){
